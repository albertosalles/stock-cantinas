...
export default function SalesHeatmap({
  ...
  const maxCents = Math.max(...slots.map(s => s.totalCents), 1);
  ...
  const pico = slots.reduce<SlotVentas | null>(
    (max, s) => (!max || s.totalCents > max.totalCents ? s : max), null);
          ...
          <div className="overflow-x-auto">
            <div style={{ minWidth: `${slots.length * 34}px` }}>
              ...
              <div className="flex h-[150px] items-end gap-[3px]">
                {slots.map(s => {
                  const alturaPct = (s.totalCents / maxCents) * 100;
                  // La opacidad codifica el calor; el mínimo evita que un tramo
                  // con pocas ventas se confunda con uno vacío.
                  const calor = s.totalCents === 0 ? 0 : 0.25 + (s.totalCents / maxCents) * 0.75;
                  return (
                    <div key={s.minuto} className="flex h-full flex-1 flex-col justify-end" title={
                      ...
                      <div
                        ...
                        style={{
                          height: `${Math.max(s.totalCents > 0 ? 4 : 2, alturaPct)}%`,
                          background: s.totalCents === 0 ? '#eef4f0' : COLOR_FASE[s.fase],
                          opacity: s.totalCents === 0 ? 1 : calor,
                        }}
                      />
                    </div>
                  );
              ...
              <div className="mt-1.5 flex gap-[3px]">
                {bandas.map((b, i) => (
                  <div key={i} style={{ flex: b.ancho }} className="min-w-0">
                    ...
                    <div className="mt-1 truncate text-center text-[9.5px] font-bold uppercase tracking-[0.04em] text-[#8aa397]">
                      {b.fase}
                    </div>
                  </div>
                ))}
